...
/* 8×8 bitmap font covering ASCII 0x20 (space) through 0x7E (~).
 * Each character is 8 bytes — one byte per row, MSB = leftmost pixel.
 * Derived from the public-domain PC BIOS ROM character set. */
...
/* Draw a character at (x, y) scaled by `scale` onto a 32-bit RGBX framebuffer.
 * stride is the number of uint32_t values per row. */
static inline void font_draw_char(uint32_t *fb, uint32_t stride,
                                  int x, int y, char c,
            ...
            if (bits & (0x80 >> col)) {
                for (int sy = 0; sy < scale; sy++)
                    for (int sx = 0; sx < scale; sx++)
                        fb[(y + row*scale + sy) * stride + (x + col*scale + sx)] = fg;
            }
        }
    }
}
...
static inline int font_draw_str(uint32_t *fb, uint32_t stride,
                                int x, int y, const char *s,
                                uint32_t fg, int scale)
{
    while (*s) {
        font_draw_char(fb, stride, x, y, *s++, fg, scale);
        x += 8 * scale;
    }
    return x;
}
